...
#define ORIGIN_LANE_BOUNDARY      0
#define LEFT_LANE_BOUNDARY        4
...
class PathPlanning 
{
  ...
  public:
    ...
    void behavioral_planning() 
    {
      if (car_front) 
      {
        // Priority for left side, If no car is on the left side, then do a left lane change
        if (!car_left && ego_car_lane > LEFT_LANE) 
        {
          ego_car_lane--;
        }
        // Else, go to right side, If no car is on the right side, then do a right lane change
        else if (!car_right && ego_car_lane != RIGHT_LANE) 
        {
          ego_car_lane++;
        }
        // If no lane change, then decrease speed for maintaining safety distance
        else 
        {
          ref_vel -= MAX_ACC;
        }
